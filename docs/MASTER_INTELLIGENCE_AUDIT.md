...
| Strategy routing | `strategy/selection.py`, `strategy/registry.py`, `analysis/opportunity.py`, `knowledge/router.py`, `analysis/regime_engine.py`, `analysis/knowledge_engine.py` | Context-aware descriptive router integrated; V2 Regime and Knowledge Selection Engines live in the live scheduler path |
| Evidence | `analysis/evidence.py`, `analysis/decision_gates.py`, `analysis/research_governance.py`, `knowledge/combinations.py`, `storage/db.py` | Independent combination comparison and additive feature/combination persistence added; feature-importance evidence is now populated from completed broker outcomes; broader combination population remains partial |
| Instrument specialization | `knowledge/specialization.py`, `knowledge/portfolio.py`, `analysis/research_governance.py`, `storage/db.py` | Separate specialization score, evidence multiplier, sample tiers, strategy/regime/timeframe fit matrix, persistent InstrumentProfiles, Core ceiling, and correlation-aware no-padding selection are implemented |
...
| Risk and sizing | `risk/manager.py`, `strategy/setup_validator.py`, `executors/mt5.py`, `analysis/capital_state.py`, `analysis/small_account.py` | Present; broker economics remain authoritative; broker quote freshness fails closed; Small Account Optimizer integrated into decision gate |
...
| Testing | `tests/*.py`, `tests/smoke_upgrade.py` | 48 tests pass plus smoke; broker freshness/metadata mocks are covered, while full walk-forward and cross-subsystem coverage remain incomplete |
...
6. Whether champion/challenger/core/quarantined promotion has explicit out-of-sample, forward-DEMO, drawdown, robustness, and rollback criteria. **Instrument specialization now computes a separate adjusted score, sample tier, fit matrix, and correlation-aware Core selection; complete realized OOS/execution/economic evidence remains required before Core promotion.**
7. Whether model drift and feature importance are persisted and can reduce authority without silently changing production behavior. **Drift classification and feature-importance storage/population now exist; automatic authority reduction remains bounded by the existing DEMO-only governance path.**
...
The current implementation slices are validated and pushed. The repository now has explicit context, knowledge, decision-record, combination-evidence, economic-regime, instrument-role, drift, Core Instrument Selection, feature-importance population, and quote-freshness contracts. Core is no longer represented by the first broker names or a filled top-ten list: it is empty until evidence, execution reliability, small-account economics, and portfolio contribution support promotion. The full specification is still broader than these slices: complete walk-forward/backtesting, weekend research orchestration, full challenger/demotion lifecycle, and end-to-end cross-subsystem broker-mock coverage remain open. Work should continue by wiring each contract to durable evidence producers and tests before any claim of institutional completeness.
